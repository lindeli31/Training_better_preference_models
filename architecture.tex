...
\begin{enumerate}[leftmargin=*, nosep]
  ...
  \item \textbf{inference\_client.py} sends async HTTP POST requests to the
        ...
        endpoint.
  \item Responses are parsed (label extraction via regex), saved to JSONL, and
        fed to \textbf{metrics.py} for analysis.
\end{enumerate}
\end{notebox}
...
\begin{description}[style=nextline, leftmargin=1.5cm]
  \item[\texttt{InferenceConfig}]
    ...
    \begin{itemize}[nosep]
      ...
      \item Thinking budget: \texttt{thinking\_budget\_tokens} (0 = disabled)
  ...
  \item[Label Extraction]
    Three-tier regex cascade:
    \begin{enumerate}[nosep]
      \item \textbf{Verdict lines}: patterns like ``Verdict: B'', ``Answer: A''
      \item \textbf{Bare label}: single A/B/C at end of text
      \item \textbf{Fallback}: last uppercase A/B/C token in text
    \end{enumerate}
    Thinking blocks (\texttt{<think>...</think>}) are stripped before label
    extraction and stored separately.
...
\begin{tabular}{lll}
  ...
  \texttt{expert\_rater\_cot} & Expert + step-by-step CoT & B3 \\
...
\begin{tabular}{llp{6.5cm}}
  ...
  \texttt{run\_thinking\_budget}     & B3 & 5 conditions: no\_thinking, cot\_prompted, think\_512, think\_1024, think\_2048 \\
...
\begin{description}[style=nextline, leftmargin=1.5cm]
  ...
  \item[\texttt{compute\_thinking\_accuracy}] Accuracy vs.\ gold labels per
    condition, agreement vs.\ no\_thinking baseline, average latency per
    condition.
\end{description}
...
python run_experiments.py --experiments position_bias --n-pairs 20

# Specific model + thinking budget
python run_experiments.py --model Qwen/Qwen3-30B-A3B-Instruct-2507 \
...
\texttt{--template}, \texttt{--thinking-budget}, \texttt{--output-dir},
...
% ── B3 ──────────────────────────────────────────────────────────────────────
\subsection{B3: Prompt-Elicited Reasoning (Thinking Budget)}\label{sec:b3}

\begin{experimentbox}[B3 -- Thinking Budget and CoT Impact]
\textbf{Research question:} Does explicit reasoning --- either prompt-elicited
Chain of Thought or native thinking tokens --- improve judge accuracy?
...
\begin{tabular}{llcp{5.8cm}}
  \toprule
  \textbf{Label} & \textbf{Template} & \textbf{Budget} & \textbf{Description} \\
  \midrule
  \texttt{no\_thinking}  & expert\_rater     & 0    & Baseline, no reasoning \\
  \texttt{cot\_prompted} & expert\_rater\_cot & 0    & Prompt instructs step-by-step reasoning; uses verbose output format \\
  \texttt{think\_512}    & expert\_rater     & 512  & Qwen3 native thinking, 512 token budget \\
  \texttt{think\_1024}   & expert\_rater     & 1024 & Qwen3 native thinking, 1024 token budget \\
  \texttt{think\_2048}   & expert\_rater     & 2048 & Qwen3 native thinking, 2048 token budget \\
  \bottomrule
\end{tabular}
\end{center}
...
\begin{description}[nosep, leftmargin=1.2cm]
  \item[\texttt{accuracy\_vs\_gold}] Agreement with human preference labels
    per condition.
  \item[\texttt{delta\_accuracy}] Improvement of each condition over
    \texttt{no\_thinking}.
  \item[\texttt{agreement\_vs\_no\_thinking}] Fraction of verdicts matching
    the baseline.
  \item[\texttt{avg\_latency\_s}] Average response time per condition (more
    thinking tokens = higher latency).
\end{description}
\end{experimentbox}

\begin{warnbox}[B3 Clarification: Prompt-Elicited CoT vs.\ Native Thinking]
B3 tests \textbf{prompt-elicited Chain of Thought} --- the model is
explicitly instructed in the prompt to ``think step by step about the
strengths and weaknesses'' before answering.

\medskip
\begin{itemize}[nosep]
  \item The \texttt{cot\_prompted} condition uses a special template
    (\texttt{expert\_rater\_cot}) with verbose output instructions
    (``Verdict: <A|B|C>'' format). The thinking budget is 0; any reasoning
    appears in the main response text.

  \item The \texttt{think\_512/1024/2048} conditions use \textbf{Qwen3's
    native thinking mode} via \texttt{chat\_template\_kwargs:
    \{enable\_thinking: true\}} and \texttt{thinking: \{type: enabled,
    budget\_tokens: N\}} in the API request body. The model produces
    \texttt{<think>...</think>} blocks that are extracted separately.

  \item This is \textbf{NOT} a native API thinking-budget feature like
    Anthropic's extended thinking. It is prompt-level CoT elicitation
    combined with Qwen3's optional thinking mode, both accessed via the
    Swiss AI Stack's OpenAI-compatible endpoint.
\end{itemize}
\end{warnbox}
...
\begin{longtable}{llp{7cm}}
  ...
  \texttt{experiment\_id} & \texttt{str} &
    ...
    \texttt{thinking\_budget}, or \texttt{input\_sensitivity}. \\
  \texttt{condition} & \texttt{str} &
    Experimental condition (e.g.\ ``AB'', ``BA'', template name,
    ``think\_1024'', ``expert\_rater\_alt2\_accurate''). \\
