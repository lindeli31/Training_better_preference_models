...
within each group. For each response, a composite quality score is computed as
\[
  s = \frac{\text{helpfulness} + \text{correctness} + \text{coherence}}{3}.
\]
Within each pair, the higher-scoring response is always placed as~A and the
lower-scoring one as~B. Consequently, the gold label is either ``A''
(A~is better) or ``C'' (tied scores) --- it is \textbf{never~``B''} at
download time. The \texttt{flipped()} method on \texttt{PairRecord} swaps
A~and~B at experiment time to test position effects (see Section~\ref{sec:b1}).
Results are saved locally as both JSON and CSV in the \texttt{data/} directory.
